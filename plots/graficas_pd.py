...
ax.set_xlabel('Número de deportistas (n)')
ax.set_ylabel('Tiempo (segundos)')
ax.set_title('Tiempos de ejecución — Programación Dinámica (W=500, K=5)')
ax.legend()
ax.grid(True, linestyle='--', alpha=0.5)
plt.tight_layout()
plt.savefig('plots/graficas/tiempos_solo_pd.png', dpi=150)
plt.close()
print("Gráfica 4 guardada: plots/graficas/tiempos_solo_pd.png")

# ── Gráfica 5: Solo Greedy ────────────────────────────────────────────
fig, ax = plt.subplots(figsize=(10, 6))
ax.plot(n_vals, t_puntaje,  marker='^', label='Greedy Mayor Puntaje',  color='#DC2626')
ax.plot(n_vals, t_costo,    marker='D', label='Greedy Menor Costo',    color='#D97706')
ax.plot(n_vals, t_ratio,    marker='x', label='Greedy Ratio',          color='#16A34A')
...
ax.set_xlabel('Número de deportistas (n)')
ax.set_ylabel('Tiempo (segundos)')
ax.set_title('Tiempos de ejecución — Algoritmos Greedy (W=500, K=5)')
ax.legend()
ax.grid(True, linestyle='--', alpha=0.5)
plt.tight_layout()
plt.savefig('plots/graficas/tiempos_solo_greedy.png', dpi=150)
plt.close()
print("Gráfica 5 guardada: plots/graficas/tiempos_solo_greedy.png")
